...
from utils import (setup_esmf_environment, cache_result, calculate_weighted_stats, create_region_mask, detrend_timeseries, calculate_pc_index_correlations, remove_global_mean)
...
def calculate_observed_odsl():
    ...
    with warnings.catch_warnings():
        ...
        if 'polyfit_coefficients' in polyfit_ds_sla:
            print('polyfit_coefficients found in altimetry dataset')
            trend_sla_alt = polyfit_ds_sla['polyfit_coefficients'].sel(degree=1)
        else:
            print('__xarray_dataarray_variable___polyfit_coefficients found in altimetry dataset')
            trend_sla_alt = polyfit_ds_sla['__xarray_dataarray_variable___polyfit_coefficients'].sel(degree=1)
        trend_sla_alt = trend_sla_alt.compute()
        ...
        #weird naming behaviour sometimes
        if 'polyfit_coefficients' in polyfit_ds_asl:
            print('polyfit_coefficients found in budget dataset')
            trend_asl_fr = polyfit_ds_asl['polyfit_coefficients'].sel(degree=1)
        elif '__xarray_dataarray_variable___polyfit_coefficients' in polyfit_ds_asl:
            print('__xarray_dataarray_variable___polyfit_coefficients found in budget dataset')
            trend_asl_fr = polyfit_ds_asl['__xarray_dataarray_variable___polyfit_coefficients'].sel(degree=1)
        else:
             print('Taking first data variable in budget dataset for trends')
             vname = list(polyfit_ds_asl.data_vars)[0]
             trend_asl_fr = polyfit_ds_asl[vname].sel(degree=1)
    ...
    for i, model_name in enumerate(TARGET_MODELS):
        ...
        if hist_file and future_file:
            ...
            try:
                ...
                slope_mm_yr = (slope * 10).where(region_mask)
                
                #variability
                detrended_data = detrend_timeseries(period_data, degree=VARIABILITY_DETREND_DEGREE, dim='time')
                with warnings.catch_warnings():
                    warnings.simplefilter("ignore", category=RuntimeWarning)
                    model_variability = detrended_data.std(dim='time').where(region_mask)
...
def perform_incrementing_window_analysis(obs_results, cmip_results):
    ...
    pcc_da = xr.DataArray([all_pcc[s] for s in source_coord], coords={'source': source_coord, 'end_year': window_end_years}, dims=['source', 'end_year'], name='pcc')
    rmse_da = xr.DataArray([all_rmse[s] for s in source_coord], coords={'source': source_coord, 'end_year': window_end_years}, dims=['source', 'end_year'], name='rmse')
...
def perform_eof_analysis(obs_results, cmip_results, n_modes=EOF_N_MODES):
    ...
    #observed data
    obs_da = obs_results['odsl_yearly'].rename({'year': 'time'})
    obs_mask = create_region_mask(obs_da.isel(time=0), EXTENT)
    sources_to_analyze['observed'] = obs_da.where(obs_mask)
    ...
    #multimodel mean
    sources_to_analyze['multi model mean'] = model_detrended.mean(dim='model')
